...
let statusBarTemporary;
let statusBarPersistent;
let vulnerabilityCounts = new Map(); // URI -> count
let persistentStatusState = new Map(); // Track state per file
let vulnLineDecorationTypes;
    ...
    context.subscriptions.push({
        dispose: () => {
            if (vulnLineDecorationTypes) {
                Object.values(vulnLineDecorationTypes).forEach(decoration => decoration.dispose());
            }
        }
    });
...
function registerCommands(context) {
    context.subscriptions.push(vscode.commands.registerCommand('secure-code-analyzer.runDynamicAnalysis', async () => await runDynamicAnalysis()), vscode.commands.registerCommand('secure-code-analyzer.cancelDynamicAnalysis', async () => await cancelDynamicAnalysis()));
    context.subscriptions.push(vscode.commands.registerCommand('secure-code-analyzer.restartAnalysis', async () => {
        await cancelDynamicAnalysis();
        await runDynamicAnalysis();
    }));
    // Conditionally register user management commands
    if (process.env.AUTH_MODE !== 'local') {
        context.subscriptions.push(vscode.commands.registerCommand('secure-code-analyzer.registerUser', async () => await registerNewUser()));
    }
}
...
async function runDynamicAnalysis() {
    ...
    try {
        ...
        const analysisId = await apiClient.runDynamicAnalysis(sourceFilePath, abortController.signal);
...
async function pollAnalysisStatus(analysisId, sourceFilePath, token) {
...
async function handleCompletedAnalysis(status, sourceFilePath) {
    try {
...
async function processCasrReports(reports, sourceFilePath) {
    const diagnostics = [];
    const sourceFileName = path.basename(sourceFilePath);
    // Prepare decoration ranges grouped by severity
    const rangesBySeverity = {
        Critical: [],
        High: [],
        Medium: [],
        Low: [],
        ...
        provideHover(document, position) {
            ...
            if (diagnostic) {
                ...
                if (report.Source) {
                    ...
                    report.Source.forEach((line) => markdown.appendMarkdown(`${line}\n`));
                ...
                if (report.Stacktrace?.length > 0) {
                    ...
                    report.Stacktrace.slice(0, 5).forEach((frame) => {
    ...
    if (diagnostics.length > 0) {
        vscode.window.showInformationMessage(`Found ${diagnostics.length} vulnerabilities in ${sourceFileName}`);
    }
}
// In applyDecorationsForFile:
function applyDecorationsForFile(filePath) {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
...
function updateTemporaryStatus(message, spinning = false) {
...
function updatePersistentStatus(stateOrCount) {
